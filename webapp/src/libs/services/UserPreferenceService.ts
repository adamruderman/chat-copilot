...
export class UserPreferenceService extends BaseService {
    private static instance = new UserPreferenceService(); // Create an instance of the service

    // Get the user's selected preference
    // Get the user's preferences
    public static getUserPreference = async (accessToken: string): Promise<UserPreference | null> => {
        try {
            const result = await this.instance.getResponseAsync<UserPreference>(
                {
                    commandPath: 'UserPreference/GetUserPreference',
                    method: 'GET',
                },
                accessToken,
            );
    ...
    public static setUserPreference = async (userPreference: UserPreference, accessToken: string): Promise<void> => {
      try {
           

            await this.instance.getResponseAsync(
                {
                    commandPath: 'UserPreference/SetUserPreference',
                    method: 'POST',
                    body: userPreference,
